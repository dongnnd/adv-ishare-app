...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        ...
        if not os.path.exists("target/release/librustdesk.dll"):
            ...
            exit(-1)
    os.chdir('flutter')
    system2('fvm flutter build windows --release')
    os.chdir('..')
    ...
    system2(
        f'python ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/NetControllIShare.exe')
    os.chdir('../..')
    ...
    os.rename('./rustdesk_portable.exe', f'./NetControlIShare-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/NetControlIShare-{version}-install.exe')
